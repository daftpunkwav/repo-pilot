...
| --------- | ---------- | --- | ---------------------------------- | ------------ | -------- |
| scout     | 快速分析 Agent | 7   | `["tools", "streaming"]`           | True         | 10       |
| mentor    | 深度讲解 Agent | 10  | `["tools", "streaming", "vision"]` | False        | 20       |
| navigator | 学习规划 Agent | 4   | `["tools", "streaming"]`           | False        | 15       |
| curator   | 分类文档 Agent | 3   | `["tools", "streaming"]`           | True         | 5        |
| scribe    | 笔记助手 Agent | 3   | `["tools", "streaming"]`           | False        | 5        |
| hub       | 对话管家       | 2   | `["tools", "streaming"]`           | False        | 0        |
...
class StreamEventType(Enum):
    """SSE 事件类型（F5-09 修复：引用 TECHNICAL_SPEC §3.5 权威定义）"""
...
class PromptGuard:
    INJECTION_PATTERNS = [
        r"ignore (all )?previous instructions", r"disregard (all )?prior",
        r"forget (everything|all)", r"you are now", r"new instructions",
        r"system prompt", r"<\|im_start\|>", r"<\|im_end\|>",
    ]
    mode: str = "block"  # S-05: block（默认拦截）/ mark（MVP 降级标记）
