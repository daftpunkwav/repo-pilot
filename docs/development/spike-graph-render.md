...
- Voyager 已有 design-system：`data-theme` dark/light + liquid-glass

## 拍板

1. **移植 R3F 栈进 Voyager 自有页面**，共享 design-system 主题，**禁止 iframe 嵌入参考 UI**。
2. **深色主题**：允许 Bloom / 后处理增强层次。
3. **浅色主题**：关闭或显著弱化 Bloom；画布/场景背景读取 CSS 变量，避免与 liquid-glass 冲突。
4. **LOD**：沿用参考策略——常规 instanced spheres；极端规模（>75k）切 point mode（当前 Voyager ~7.4k 节点，默认走实例化球体即可）。
