 ...
 */
export interface SseHandlerCtx {
  set: (partial: object | ((s: object) => object)) => void;
  get: () => object;
}
...
export const handleSubagentThinking: SseHandler = (event, ctx) => {
  ...
  const agentId = (typeof raw.agent_id === 'string' ? raw.agent_id : '') as any;
  const content = typeof raw.content === 'string' ? raw.content : '';
  if (!agentId || !content) return;
  ctx.set((state: any) => ({
    subagents: state.subagents.map((s: any) =>
      s.agentId === agentId
 ...
 *  主体内；后续 PR 跟进。
 */
export const HANDLERS: Record<string, SseHandler> = {
