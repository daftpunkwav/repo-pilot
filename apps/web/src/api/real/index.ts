/**
 * RealApiClient — 真实后端 IApiClient 实现,按业务域拆分后通过 7 个 Api 子类委托 (§4.2.17)
 *
 * 委托(而非继承)的好处:
 *   1. 每个子域独立维护、可单测;主体仅保留 7 个 readonly 域字段。
 *   2. 类型安全:methods 直接转发到 `this.{auth,projects,...}`,
 *      TS 在编译期验证 IApiClient 全部签名(否则 implements 失败)。
 *   3. 兼容 IApiClient:`import { RealApiClient } from '@/api/real'` 路径、
 *      `new RealApiClient()` 形态、`getApi().login(p)` 调用全部不变。
 */
